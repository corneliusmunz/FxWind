...
// windspeed in m/s
void WindSpeed::calculateWindspeed()
{
...
WindspeedEvaluation WindSpeed::evaluateWindspeed()
{
    ...
    evaluationResult.AverageWindspeed = (float)(sumWindspeed / _evaluationRange) / 10.0f;
    for (size_t i = 0; i < 15; i++)
    {
...
String WindSpeed::getWindspeedString(float windspeedValue, bool addUnitSymbol = false)
{
...
void WindSpeed::logWindspeedToSDCard(fs::FS &fs, float windspeed)
{
    if (!fs.exists(getLogFilePath().c_str()))
    {
        writeLineToFile(fs, getLogFilePath().c_str(), getLogFileHeader().c_str());
    }
    appendLineToFile(fs, getLogFilePath().c_str(), getLogCsvRow(windspeed).c_str());
}

String WindSpeed::getWindspeedEvaluationString(WindspeedEvaluation windspeedEvaluation)
{
